...
def fetch():
    try:
        ...
        params = {
            ...
            "data_id": "TAIEX",  # 台灣加權指數
            "start_date": "2025-01-01",
        }
        resp = requests.get(url, params=params).json()
        latest_price = round(resp["data"][-1]["close"], 1)
        return latest_price
    except:
        return 21000.0
...
def main():
    ...
    strike_default = (100 - ((fetch()) % 100)) + fetch()
    strike = st.sidebar.number_input("Strike Price", value = strike_default, step = 5.0, format = "%0.1f")
    spot = st.sidebar.number_input("Spot Price of Underlying (default is TWII)", value = fetch(), step = 5.0, format = "%0.1f")    
...
# streamlit run c:\Users\User\OneDrive\桌面\git-repos\Option\BlackScholesCalculator.py
